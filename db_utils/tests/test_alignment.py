...
class TestTranscriptionTranslationAlignment(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.engine = create_engine("sqlite:////Users/mrgecko/Documents/Dev/Data/adele/adele.sqlite")
    ...
    def test_alignment_line_count(self):
        # we expect the same line count as in the original doc
        translation_verses_count = len(self.doc.xpath("//ti:div[@type='translation']//ti:l[normalize-space()]", namespaces=NS_TI))
        transcription_verses_count = len(self.doc.xpath("//ti:div[@type='transcription']//ti:l[normalize-space()]", namespaces=NS_TI))

        self.assertEqual(translation_verses_count, len(self.res), "Ptr count is incorrect")
        self.assertEqual(transcription_verses_count, len(self.res), "Ptr count is incorrect")
    ...
    def test_alignment_is_correct(self):
        with open("./data/transcription_doc20.txt", "r") as transcription_f, \
            ...
            self.assertEqual(len(transcription_lines), len(translation_lines),
                             "Line count is wrong. Check the integrity of your test dataset")
